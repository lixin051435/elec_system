...
class SudokuDemo implements ActionListener, FocusListener, KeyListener {
	...
	SudokuDemo() {
		...
		try {
			sudokuResources = ResourceBundle
					.getBundle(
		...
		techniquesButton.setToolTipText(sudokuResources
		...
		JLabel instructions = new JLabel("<html><p style=\"font-size:95%\">"
				+ sudokuResources.getString("Instructions") + "</p><br>");
		...
		/* =============================== */
			...
			for (int i = 1; i <= tNum; i++) {
				...
				messageStr = messageStr
						+ ((NumberValue) fv.getFactSlot("priority")).intValue()
						+ ". "
						+ ((LexemeValue) fv.getFactSlot("reason"))
								.lexemeValue() + "<br>";
			}
